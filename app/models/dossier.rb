class Dossier < ApplicationRecord
  ...
  scope :with_notifications, -> do
    ...
    # However, it’s better to be able to call Dossier.with_notifications on its own (even if it’s a bit weird). Let’s just LEFT OUTER JOIN.
    joined_dossiers = left_outer_joins(:follows)

    # Relations passed to #or must be “structurally compatible”, i.e. query the same tables.
    joined_dossiers = joined_dossiers.left_outer_joins(:champs, :champs_private, :avis, :commentaires)
